...
public class Frequencer implements FrequencerInterface{
  ...
  private int subByteStartIndex(int start, int end) {
    ...
    if(end > mySpace.length) {
      return -1;
    }
    String str = new String(mySpace);
    String subBS = str.substring(start,end); //subByteString
    for(int i = 0 ;i < suffixArray.length ;i++) {
      if(str.substring(suffixArray[i], mySpace.length).startsWith(subBS)) {
        return i;
      }
    }
    return -1;
  ...
  private int subByteEndIndex(int start, int end) {
    ...
    for(int i = 0 ;i < suffixArray.length ;i++){
      startB = str.substring(suffixArray[i], mySpace.length).startsWith(subBS);
      if(startB == true){
        subBF = true;
      }
  ...
  public static void main(String[] args){
    ...
    try{
      ...
      System.out.println(frequencerObject.subByteStartIndex(3,5));
      System.out.println(frequencerObject.subByteStartIndex(3,6));
      System.out.println(frequencerObject.subByteStartIndex(3,30));
      System.out.println(frequencerObject.subByteEndIndex(3,5));
      System.out.println(frequencerObject.subByteEndIndex(3,6));
